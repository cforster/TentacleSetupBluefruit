...
void setup() {
  ...
  if ( !ble.begin(VERBOSE_MODE) )
    ...
    while (! ble.isConnected()) {
      delay(500);
      ble.println("connecting to BLE");
    }
    /* Set module to DATA mode */
    ble.setMode(BLUEFRUIT_MODE_DATA);
    delay(3000); //this guarantees that the first data is sent.  todo: sync this correctly.
...
void loop() {
  ...
  while (ble.available() > 0) {              // While there is serial data from the computer

    computer_in_byte = ble.read();           // read a byte
    ...
    if (String(cmd) == F("help")) {             //if help entered...
      ...
      for (int x = 0; x <= 127; x++) {    //loop through input searching for a channel change request (integer between 1 and 127)
        if (String(cmd) == String(x)) {
          ble.println(F("changing channel to "));
          ble.println( cmd);
          ...
          if (change_channel()) {         //set MUX switches or I2C address
            ...
            ble.println(  F("-------------------------------------"));
            ble.println(    F("ACTIVE channel : "));
            ble.println(  channel );
            ble.println(    F("Type: "));
            ble.println(    stamp_type);
            ble.println(    F(", Version: "));
            ble.println(    stamp_version);
            ble.println(    F(", COM: "));
            ...
            if (channel > 8) {
              ...
              ble.println(  F("UART ("));
              ble.println(  String(channelBaudrate[channel]));
              ble.println(F(" baud)"));
            }
#else
            ble.println(F("I2C"));
            ...
            ble.println(F("CHANNEL NOT AVAILABLE! Empty slot? Different COM-mode?"));
            ble.println(F("Try 'scan' or set baudrate manually (see 'help')."));
          }
          computer_msg_complete = false;                    //Reset the var computer_bytes_received to equal 0
          return;
        }
      ...
      for ( int x = 0; x < 5; x++) {                    // check if a baudrate was entered manually
        if (String(cmd) == String(validBaudrates[x])) {
          ble.println(F("Setting baudrate manually to "));
          ble.println(validBaudrates[x]);
      ...
      if (String(cmd).startsWith(F("serial,")) || String(cmd).startsWith(F("i2c,"))) {
        ble.println(F("! when switching from i2c to serial or vice-versa,"));
        ble.println(F("! don't forget to switch the hardware jumpers accordingly."));
      }

    }

    ble.println(F("> "));                              // echo to the serial console
    ble.println(cmd);


    if (I2C_mode == false) {                          //if serial channel selected
#if !defined (I2C_ONLY)
      sSerial.println(cmd);                             //Send the command from the computer to the Atlas Scientific device using the softserial port
      sSerial.println("\r");                            //After we send the command we send a carriage return <CR>
#endif
    }
      ...
      if (sensor_bytes_received > 0) {
        ble.println(F("< "));
        ble.println(sensordata);       //print the data.
      }
    }
  ...
  if (sSerial.available() > 0) {                          //If data has been transmitted from an Atlas Scientific device
    ...
    ble.println(F("< "));
    ble.println(sensordata);                           //let’s transmit the data received from the Atlas Scientific device to the serial monitor
  }
#endif
}
...
byte I2C_call() {           //function to parse and call I2C commands.
  ...
  //ble.println("beginning transmission... ");

  WIRE.beginTransmission(channel);                  //call the circuit by its ID number.
  //ble.println("transmit command: ");
  //ble.println(cmd);
  WIRE.write(cmd);                                  //transmit the command that was sent through the serial port.
  //ble.println("... ending transmission");
  error = WIRE.endTransmission();                           //end the I2C data transmission.

  //ble.println("command sent");
  
  if (error != 0) {
    //ble.println("i2c error ");
    //ble.println(error);
  }
  ...
  while (i2c_response_code == 254) {      // in case the cammand takes longer to process, we keep looping here until we get a success or an error
    ...
    //ble.println("requesting data from device");
    ...
    //ble.println("response code: ");
    //ble.println(i2c_response_code);
    ...
    switch (i2c_response_code) {         //switch case based on what the response code is.
      case 1:                          //decimal 1.
        ble.println( F("< success"));     //means the command was successful.
        break;                           //exits the switch case.

      case 2:                          //decimal 2.
        ble.println( F("< command failed"));     //means the command has failed.
        break;                           //exits the switch case.

      case 254:                        //decimal 254.
        ble.println( F("< command pending"));    //means the command has not yet been finished calculating.
        break;                           //exits the switch case.

      case 255:                        //decimal 255.
        ble.println( F("< no data"));    //means there is no further data to send.
        break;                           //exits the switch case.
    }
  }
}
...
boolean scan_baudrates() {                               // scans baudrates to auto-detect the right one for this uart channel. if one is found, it is saved globally in channelBaudrate[]
  ...
  for (int j = 0; j < 5; j++) {
    ...
    sSerial.println(F("c,0\r"));                            // switch off continuous mode for new ezo-style stamps
    delay(150);
    //clearIncomingBuffer();                             // buffers are full of junk, clean up
    sSerial.println(F("e\r"));                              // switch off continous mode for legacy stamps
...
boolean parseInfo() {                  // parses the answer to a "i" command. returns true if answer was parseable, false if not.
  ...
  /*
    ble.println("can not parse data: ");
    ble.println("'");
    ble.println(sensordata);
    ble.println("'");
  */
...
void scan(boolean scanserial) {                      // Scan for all devices. Set scanserial to false to scan I2C only (much faster!)
...
#if !defined (I2C_ONLY)
  if (scanserial) {
    ble.println(F("Starting scan... this might take up to a minute."));
    ble.println(F("(if only using i2c mode, use 'scani2c' to scan faster)"));
  } else {
    ble.println(F("Starting  I2C scan..."));
  }
#endif
  ...
  if (scanserial) {
    for (channel = 0; channel < 8; channel++) {
      ...
      if (change_channel()) {
        ...
        serialPrintDivider();
        ble.println(    F("-- SERIAL CHANNEL "));
        ble.println(  channel);
        ble.println(  F("--"));
        ble.println(    F("-- Type: "));
        ble.println(  stamp_type);
        ble.println(    F("-- Baudrate: "));
        ble.println(  channelBaudrate[channel]);
      }
    }
  }
  ...
  ble.println(  F("\r\r"));
  ble.println(  F("SCAN COMPLETE"));
  ble.println(    stamp_amount);
  ble.println(  F(" stamps found"));
}
...
void intro() {                                             //print intro
  serialPrintDivider();
  ble.println( F("Whitebox Labs -- Tentacle Shield - Stamp Setup"));
  ble.println( F("For info type 'help'"));
  ble.println( F("To read current config from attached stamps type 'scan'"));
  ble.println( F(" (e.g. if you've changed baudrates)"));
  ble.println( F("To read current config from attached I2C stamps only, type 'scani2c'"));
#if !defined (I2C_ONLY)
  ble.println( F("TYPE CHANNEL NUMBER (Serial: 0-7, I2C: 8-127):"));
#else
  ble.println( F("TYPE CHANNEL NUMBER (I2C: 0-127):"));
#endif
}

void help() {                                            //print help dialogue
  serialPrintDivider();
  ble.println( F("To open a serial channel (numbered 0 - 7), send the number of the channel"));
  ble.println( F("To open a I2C address (between 8 - 127), send the number of the address"));
  ble.println( F("To issue a command, enter it directly to the console."));
  ble.println( F("To update information about connected stamps, type 'scan'."));
  ble.println( F(" -> This might take a while, it will scan all ports (serial and I2C)"));
  ble.println( F(" -> and determine correct baudrates."));
  ble.println( F("To set baudrate manually, type one of 38400,19200,9600,115200,57600"));
  ble.println( F("=========="));
}
